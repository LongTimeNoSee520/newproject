 ...
 */
@RestController
@RequestMapping("tWUseWaterOriginalPlan")
...
public class UseWaterOriginalPlanController {
	...
	public ApiResponse queryPage(@RequestBody JSONObject jsonObject) {
		...
		if (null != jsonObject) {
			try {
				Page<UseWaterOriginalPlan> result = useWaterOriginalPlanService.queryPage(jsonObject);
	...
	public ApiResponse add(@RequestHeader("token")String token,@RequestBody JSONObject jsonObject) {
		...
		if (null != jsonObject) {
			try {
				boolean result = useWaterOriginalPlanService.saveModel(jsonObject);
				if (result) {
					apiResponse.setMessage(ResponseMsgConstants.OPERATE_SUCCESS);
				} else {
					apiResponse.recordError(ResponseMsgConstants.OPERATE_FAIL);
	...
	public ApiResponse edit(@RequestHeader("token")String token,@RequestBody JSONObject jsonObject) {
		...
		if (null != jsonObject) {
			try {
				boolean result = useWaterOriginalPlanService.updateModel(jsonObject);
				if (result) {
					apiResponse.setMessage(ResponseMsgConstants.OPERATE_SUCCESS);
				} else {
					apiResponse.recordError(ResponseMsgConstants.OPERATE_FAIL);
	...
	/**
	 * 通过id删除TWUseWaterOriginalPlan数据方法
	 * @param
	 * @return
	 */
    @ResponseBody
	...
	public ApiResponse remove(@RequestHeader("token") String token, @RequestBody JSONObject jsonObject) {
		...
		if (null != jsonObject) {
			try {
				boolean result = useWaterOriginalPlanService.deleteModel(jsonObject);
				if (result) {
					apiResponse.setMessage(ResponseMsgConstants.OPERATE_SUCCESS);
				} else {
					apiResponse.recordError(ResponseMsgConstants.OPERATE_FAIL);
